...
import {
  ...
  type CreateRatePlanSchemaType,
} from '../schemas/createRatePlanSchema';
...
export function RatePlanFormPage({ utilityType }: RatePlanFormPageProps) {
  ...
  const form = useForm<
    z.input<typeof createRatePlanSchema>,
    unknown,
    CreateRatePlanSchemaType
  >({
    ...
    try {
      const ratePlan = await createRatePlan({
        createRatePlanCommand: {
          ...
          fixedAmount:
            values.structure === 'Fixed' ? (values.fixedAmount ?? 0) : null,
      ...
      navigate(
        `/utilities/${utilityType.toLowerCase()}/rate-plans?buildingId=${ratePlan.buildingId}`,
      );
    } catch (err) {
    ...
    <>
      <PageHeader
        ...
        crumbs={[
          ...
          {
            label: 'Rate Plans',
            path: `/utilities/${utilityType.toLowerCase()}/rate-plans`,
          },
          { label: 'New' },
        ]}
      />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder={`e.g. Standard ${utilityType} Rate`}
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
