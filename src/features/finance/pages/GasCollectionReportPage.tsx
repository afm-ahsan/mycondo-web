import { Flame, HandCoins, Printer, Undo2 } from 'lucide-react';
...
import { useGasCollectionReport } from '../api/reportsApi';
...
/**
 * Period-scoped Billed/Collected/Waived for gas invoices. Billed and Collected are kept as separate
 * KPI cards with distinct tones (Template 5's "Permanent Financial Distinctions" rule — Billed/Earned
 * and Collected must never be merged into one figure).
 */
export function GasCollectionReportPage() {
  const [filters, setFilters] = useUrlFilters(FILTER_DEFAULTS);

  const { data, isFetching, isError, refetch } = useGasCollectionReport({
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        crumbs={[{ label: 'Finance' }, { label: 'Reports' }, { label: 'Gas Collection' }]}
      />
      ...
      {isError ? (
        <ErrorState description="Couldn't load the gas collection report." onRetry={refetch} />
      ) : (
