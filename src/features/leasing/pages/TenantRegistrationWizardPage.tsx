...
export function TenantRegistrationWizardPage() {
  ...
  const [registrationId, setRegistrationId] = useState<string | undefined>(routeId);
  ...
  function handleOccupancyDetailsSaved(id: string, values: OccupancyDetailsSchemaType) {
  ...
  return (
    <>
      <PageHeader
        ...
        crumbs={[{ label: 'Tenant Registrations', path: '/leasing/tenant-registrations' }, { label: registrationId ? 'Edit' : 'New' }]}
      />
      <Card className="mx-auto max-w-3xl">
        ...
        <Stepper value={activeStep} onValueChange={goToStep}>
          <StepperNav className="mb-6">
            {STEPS.map(({ step, title }, index) => (
              <StepperItem key={step} step={step} disabled={step > activeStep && !registrationId}>
                <StepperTrigger>
        ...
        {activeStep === 1 && (
          <OccupancyDetailsStep
            ...
            defaultValues={{
              ...
              occupancyType: registration?.occupancyType as 'Owner' | 'Occupant' | undefined,
              primaryFullName: registration?.primaryFullName ?? primaryFullName,
              moveInExpectedDate: registration?.moveInExpectedDate ?? undefined,
            }}
        ...
        {activeStep === 2 && registrationId && (
          <PrimaryResidentStep
            ...
            defaultValues={{
              ...
              primaryDateOfBirth: registration?.primaryDateOfBirth ?? undefined,
              primaryPermanentAddress: registration?.primaryPermanentAddress ?? undefined,
              emergencyContactName: registration?.emergencyContactName ?? undefined,
              emergencyContactPhone: registration?.emergencyContactPhone ?? undefined,
            }}
            onSaved={() => goToStep(3)}
            onBack={() => goToStep(1)}
          />
        ...
        {activeStep === 4 && registrationId && (
          <DocumentsStep registrationId={registrationId} onContinue={() => goToStep(5)} onBack={() => goToStep(3)} />
        )}

          {activeStep === 5 && registrationId && (
            <ReviewSubmitStep registrationId={registrationId} onBack={() => goToStep(4)} />
          )}
        </CardContent>
      </Card>
    </>
