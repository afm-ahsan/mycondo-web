...
import {
  useApproveProviderAssignment,
  useCreateProviderAssignment,
  useDeactivateProviderAssignment,
...
import { PROVIDER_STATUSES } from '../lib/constants';
...
interface ProviderManageDialogProps {
  provider: ServiceProviderProfileDto | null;
  onOpenChange: (open: boolean) => void;
}

export function ProviderManageDialog({ provider, onOpenChange }: ProviderManageDialogProps) {
  return (
          ...
          <>
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                {provider.fullName}
                <StatusBadge status={provider.status as 'Active' | 'Suspended' | 'Blocked'} toneMap={statusToneMap} />
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-6">
              <StatusSection provider={provider} />
              <Separator />
              <AssignmentsSection provider={provider} />
            </div>
          </>
        )}
      </DialogContent>
...
function StatusSection({ provider }: { provider: ServiceProviderProfileDto }) {
  const [setStatus, { isLoading }] = useSetProviderStatus();
  ...
  async function handleSubmit() {
    ...
    try {
      await setStatus({
        id: provider.serviceProviderProfileId,
        setServiceProviderStatusRequest: { status: nextStatus, reason: reason || null },
      }).unwrap();
    ...
    <RequirePermission permission={PERMISSIONS.serviceProvider.manage}>
      <div className="space-y-3">
        ...
        {provider.statusReason && (
          <p className="text-muted-foreground text-sm">Current reason: {provider.statusReason}</p>
        )}
        {error && (
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <Select value={nextStatus} onValueChange={setNextStatus}>
            ...
            <SelectContent>
              {PROVIDER_STATUSES.filter((s) => s !== provider.status).map((s) => (
...
function AssignmentsSection({ provider }: { provider: ServiceProviderProfileDto }) {
  const { data: assignments, isLoading, isFetching } = useProviderAssignments({
    id: provider.serviceProviderProfileId,
  });
  ...
  return (
    <div className="space-y-3">
      ...
      {showAddForm && (
        <AddAssignmentForm
          serviceProviderProfileId={provider.serviceProviderProfileId}
      ...
      {isLoading ? (
        ...
        <ul className="space-y-2">
          {assignments.map((assignment) => {
            ...
            return (
              <li
                key={assignment.serviceProviderAssignmentId}
...
function AddAssignmentForm({
  ...
  const [createAssignment, { isLoading }] = useCreateProviderAssignment();
  ...
  async function handleSubmit() {
    ...
    try {
      await createAssignment({
        id: serviceProviderProfileId,
        createServiceProviderAssignmentRequest: {
          flatId,
